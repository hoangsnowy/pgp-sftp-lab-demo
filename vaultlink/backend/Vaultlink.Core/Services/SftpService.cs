...
public class SftpService
{
    public Task<bool> TestSshConnectionAsync(SshTestRequest request)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(request.PrivateKeyPem))
            {
                using var keyStream = new MemoryStream(System.Text.Encoding.UTF8.GetBytes(request.PrivateKeyPem));
                var privateKeyFile = new PrivateKeyFile(keyStream);
                connectionInfo = new ConnectionInfo(request.Host, request.Port, request.User, new PrivateKeyAuthenticationMethod(request.User, privateKeyFile));
            }
            else if (!string.IsNullOrEmpty(request.PrivateKeyPath) && File.Exists(request.PrivateKeyPath))
            {
                var privateKeyFile = new PrivateKeyFile(request.PrivateKeyPath);
                connectionInfo = new ConnectionInfo(request.Host, request.Port, request.User, new PrivateKeyAuthenticationMethod(request.User, privateKeyFile));
            }
            else
            {
                throw new ArgumentException("Either PrivateKeyPem or valid PrivateKeyPath must be provided");
            }
